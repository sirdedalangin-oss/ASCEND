...
class AscendApiTest extends TestCase
{
    ...
    public function test_upload_creates_an_automatic_framework_assessment(): void
    {
        Storage::fake('public');
        $token = $this->registerToken();
        ...
        $submission = $this->withToken($token)->postJson('/api/manuscripts/submit', [
            ...
            ->assertJsonPath('status', 'evaluated')
            ->assertJsonPath('evaluation_method', 'automatic_manuscript_review')
            ->assertJsonStructure(['framework_ratings', 'framework_score', 'framework_level', 'framework_notes']);

        $assessment = (new ScalabilityFramework)->assess($submission->json('framework_ratings'));
        $this->assertSame($assessment['score'], (int) $submission->json('framework_score'));
        $this->assertSame($assessment['level'], $submission->json('framework_level'));
        $this->assertStringContainsString('Automatic manuscript review', $submission->json('framework_notes'));
        ...
        $this->withToken($token)->getJson('/api/evaluations')
            ->assertOk()
            ->assertJsonPath('0.id', $submission->json('innovation_id'));
    }

    public function test_manuscript_file_and_metadata_can_be_submitted_together(): void
    {
        Storage::fake('public');
        $token = $this->registerToken();
        $keyword = $this->withToken($token)->postJson('/api/keywords', [
        ...
        $file = UploadedFile::fake()->createWithContent(
            'project-reading.txt',
            'Project Reading is a literacy intervention that supports learners through guided reading, regular practice, teacher monitoring, and progress checks.'
        );

        $submission = $this->withToken($token)->post('/api/manuscripts/submit', [
            'file' => $file,
            'key_stage' => 'KS1',
            ...
            ->assertJsonPath('detected_title', 'project reading')
            ->assertJsonPath('detected_learning_focus', 'literacy');

        $innovation = Innovation::findOrFail($submission->json('innovation_id'));
        $this->assertSame('Maria Santos', $innovation->author);
        $this->assertSame('evaluated', $innovation->status);
        $this->assertNotNull($innovation->framework_score);
        $this->assertStringContainsString('/storage/manuscripts/', $innovation->manuscript_file_url);
        Storage::disk('public')->assertExists('manuscripts/'.basename($innovation->manuscript_file_url));
        $this->assertDatabaseHas('innovation_keyword', [
    ...
    public function test_administrator_can_correct_automatic_ratings(): void
    {
        Storage::fake('public');
        ...
        $submission = $this->withToken($token)->post('/api/manuscripts/submit', [
            'file' => UploadedFile::fake()->createWithContent(
                'reading-plan.txt',
                'A guided reading intervention supports struggling Grade 1 learners through daily practice and teacher observation. The school records baseline reading needs and plans to monitor progress every week.'
            ),
            'key_stage' => 'KS1',
            'grade_levels' => 'Grade 1',
        ])->assertCreated()
        ...
        $id = $submission->json('innovation_id');
        $this->withToken($token)->putJson("/api/evaluations/{$id}", $this->assessmentPayload($id, [
            'division_impact' => 5,
            'context_adaptability' => 5,
            'adoption_ease' => 5,
            'sustainability' => 5,
            'resource_efficiency' => 5,
        ]))->assertOk()
            ->assertJsonPath('framework_score', 100)
            ->assertJsonPath('framework_level', 'Platinum Scalable Innovation')
            ->assertJsonPath('evaluation_method', 'manual_panel_evaluation')
            ->assertJsonPath('framework_ratings.division_impact', 5)
            ->assertJsonPath('recommendation', 'The corrected framework ratings meet the scalability threshold. Review the rating notes for supporting evidence.');
    ...
    public function test_revision_updates_the_existing_assessment_without_creating_another(): void
    {
        Storage::fake('public');
        $uploaderToken = $this->registerToken();
        $otherToken = $this->registerToken();
        $first = $this->withToken($uploaderToken)->post('/api/manuscripts/submit', [
            'file' => UploadedFile::fake()->createWithContent(
                'reading-draft.txt',
                'This reading intervention identifies a baseline learning gap for Grade 1 pupils. Teachers use activity sheets and observe learning progress each week.'
            ),
            'key_stage' => 'KS1',
            'grade_levels' => 'Grade 1',
        ])->assertCreated();
        $id = $first->json('innovation_id');
        $original = Innovation::findOrFail($id);
        $oldFile = 'manuscripts/'.basename($original->manuscript_file_url);

        $this->withToken($otherToken)->post("/api/manuscripts/{$id}/revise", [
            'file' => UploadedFile::fake()->createWithContent('unauthorized.txt', str_repeat('Reading intervention. ', 8)),
        ])->assertForbidden();

        $revised = $this->withToken($uploaderToken)->post("/api/manuscripts/{$id}/revise", [
            'file' => UploadedFile::fake()->createWithContent(
                'reading-revision.txt',
                'Baseline learning gap results showed improvement across schools. The contextualized activity sheets include an implementation guide, teacher training, monitoring and evaluation, and reusable low-cost materials.'
            ),
        ])->assertOk()->assertJsonPath('id', $id)
            ->assertJsonPath('evaluation_method', 'automatic_manuscript_review');

        $this->assertDatabaseCount('innovations', 1);
        $this->assertNotSame($original->manuscript_file_url, $revised->json('manuscript_file_url'));
        $this->assertNotSame($original->framework_score, $revised->json('framework_score'));
        Storage::disk('public')->assertMissing($oldFile);
        Storage::disk('public')->assertExists('manuscripts/'.basename($revised->json('manuscript_file_url')));
        $this->withToken($uploaderToken)->getJson('/api/evaluations')->assertOk()->assertJsonCount(1);
    }

    public function test_unreadable_revision_keeps_the_existing_manuscript_and_assessment(): void
    {
        Storage::fake('public');
        $token = $this->registerToken();
        $first = $this->withToken($token)->post('/api/manuscripts/submit', [
            'file' => UploadedFile::fake()->createWithContent(
                'original.txt',
                'This literacy intervention addresses a baseline learning gap. Teachers use activity sheets, a weekly schedule, and progress monitoring to support Grade 1 learners.'
            ),
            'key_stage' => 'KS1',
            'grade_levels' => 'Grade 1',
        ])->assertCreated();
        $original = Innovation::findOrFail($first->json('innovation_id'));

        $this->withToken($token)->post("/api/manuscripts/{$original->id}/revise", [
            'file' => UploadedFile::fake()->createWithContent('empty.txt', 'Too short.'),
        ])->assertStatus(500);

        $this->assertSame($original->manuscript_file_url, $original->fresh()->manuscript_file_url);
        $this->assertSame($original->framework_ratings, $original->fresh()->framework_ratings);
        $this->assertCount(1, Storage::disk('public')->allFiles('manuscripts'));
        $this->assertDatabaseCount('innovations', 1);
    }

    public function test_uploader_can_correct_own_ratings_but_cannot_validate_them(): void
    {
        Storage::fake('public');
        $uploaderToken = $this->registerToken();
        $otherUserToken = $this->registerToken();
        $adminToken = $this->adminToken();
        $submission = $this->withToken($uploaderToken)->post('/api/manuscripts/submit', [
            'file' => UploadedFile::fake()->createWithContent(
                'numeracy-plan.txt',
                'This numeracy intervention supports struggling Grade 1 learners. The manuscript describes baseline assessment, weekly practice, teacher training, an implementation guide, and monitoring for improvement.'
            ),
            'key_stage' => 'KS1',
            'grade_levels' => 'Grade 1',
        ])->assertCreated();
        ...
        $id = $submission->json('innovation_id');
        $payload = $this->assessmentPayload($id, array_fill_keys(array_keys(ScalabilityFramework::CRITERIA), 4));

        $this->withToken($otherUserToken)->putJson("/api/evaluations/{$id}", $payload)->assertForbidden();
        $this->withToken($uploaderToken)->putJson("/api/evaluations/{$id}", [
            ...$payload,
            'status' => 'validated',
        ])->assertForbidden();

        $this->withToken($uploaderToken)->putJson("/api/evaluations/{$id}", $payload)
            ->assertOk()
            ->assertJsonPath('framework_score', 80)
            ->assertJsonPath('evaluation_method', 'manual_owner_correction')
            ->assertJsonPath('status', 'evaluated')
            ->assertJsonPath('title', 'numeracy plan')
            ->assertJsonPath('school', null);

        $this->withToken($adminToken)->putJson("/api/evaluations/{$id}", [
            ...$payload,
            'status' => 'validated',
        ])->assertOk()->assertJsonPath('status', 'validated');
        $this->withToken($uploaderToken)->putJson("/api/evaluations/{$id}", $payload)->assertForbidden();
    }

    public function test_uploader_can_remove_an_unvalidated_automatic_submission(): void
    {
        Storage::fake('public');
        $uploaderToken = $this->registerToken();
        $otherUserToken = $this->registerToken();
        $submission = $this->withToken($uploaderToken)->post('/api/manuscripts/submit', [
            'file' => UploadedFile::fake()->createWithContent(
                'reading-check.txt',
                'This reading intervention addresses baseline learning gaps among Grade 1 learners. Teachers use guided activities, assessment records, and a weekly monitoring plan to track progress.'
            ),
            'key_stage' => 'KS1',
            'grade_levels' => 'Grade 1',
        ])->assertCreated();
        ...
        $id = $submission->json('innovation_id');
        $this->withToken($otherUserToken)->deleteJson("/api/innovations/{$id}")->assertForbidden();
        $this->withToken($uploaderToken)->deleteJson("/api/innovations/{$id}")->assertOk();
        $this->assertDatabaseMissing('innovations', ['id' => $id]);
    }

    public function test_manuscript_submission_returns_422_when_file_and_url_are_missing(): void
    {
        $token = $this->registerToken();
        ...
        $this->withToken($token)->postJson('/api/manuscripts/submit', [
            ...
            ->assertJsonPath('errors.file.0', 'Upload a manuscript file and try again.');

        $this->assertDatabaseCount('innovations', 0);
    }

    public function test_unreadable_manuscript_does_not_leave_a_stored_file_or_innovation(): void
    {
        Storage::fake('public');
        $token = $this->registerToken();

        $this->withToken($token)->post('/api/manuscripts/submit', [
            'file' => UploadedFile::fake()->createWithContent('unreadable.txt', 'Too short.'),
            'key_stage' => 'KS1',
            'grade_levels' => 'Grade 1',
        ])->assertStatus(500);

        $this->assertSame([], Storage::disk('public')->allFiles('manuscripts'));
        $this->assertDatabaseCount('innovations', 0);
    }
    ...
    public function test_searchable_pdf_receives_automatic_framework_ratings(): void
    {
        Storage::fake('public');
        $token = $this->registerToken();
        $file = UploadedFile::fake()->createWithContent(
        ...
        $this->withToken($token)->postJson('/api/manuscripts/submit', [
            ...
            ->assertJsonPath('status', 'evaluated')
            ->assertJsonPath('evaluation_method', 'automatic_manuscript_review')
            ->assertJsonStructure(['framework_ratings', 'framework_score', 'framework_level']);
    }
