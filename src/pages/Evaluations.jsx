...
import { Eye, FileCheck2, Loader2, Pencil, Plus, Search, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="page-shell space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <p className="page-eyebrow">Panel review</p>
          <h1 className="page-title">Scalability Assessments</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Five panel ratings determine the score and classification under the Innovation Scalability Framework.
          </p>
        </div>
        {canAssess && <Button onClick={openCreate}><Plus className="w-4 h-4 mr-2" /> New Assessment</Button>}
      </div>

      {!canAssess && <Card className="p-4 text-sm text-muted-foreground">Only administrators can record or validate assessments.</Card>}
          ...
          <div className="divide-y divide-border md:hidden">
            {filtered.map((evaluation) => (
              <article key={evaluation.id} className="min-w-0 space-y-3 p-4">
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground"><span>{evaluation.framework_level}</span><span>{evaluation.status === 'validated' ? 'Panel validated' : 'Assessed'}</span><span>{evaluation.framework_assessed_at ? new Date(evaluation.framework_assessed_at).toLocaleDateString() : '—'}</span></div>
                <div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" asChild><Link to={`/innovations/${evaluation.id}`}><Eye className="h-4 w-4" /> View</Link></Button>{canAssess && <><Button variant="outline" size="sm" onClick={() => openEdit(evaluation)}><Pencil className="h-4 w-4" /> Edit</Button><Button variant="outline" size="sm" onClick={() => setDeleteTarget(evaluation)} className="text-rose-700"><Trash2 className="h-4 w-4" /> Remove</Button></>}</div>
              </article>
            ))}
          </div>
            ...
            <TableBody>
              {filtered.map((evaluation) => (
                <TableRow key={evaluation.id}>
                  ...
                  <TableCell className="text-xs">{evaluation.status === 'validated' ? 'Panel validated' : 'Assessed'}</TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {canAssess && <>
                        <Button variant="ghost" size="icon" onClick={() => openEdit(evaluation)} title="Edit assessment"><Pencil className="w-4 h-4" /></Button>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[92dvh] w-[calc(100vw-2rem)] max-w-4xl overflow-x-hidden overflow-y-auto">
          <form onSubmit={saveEvaluation} className="space-y-6">
            <DialogHeader>
              ...
              <DialogDescription>Rate each framework criterion from 1 to 5. The score and classification are calculated from the fixed weights.</DialogDescription>
            </DialogHeader>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div><Label htmlFor="assessment-author">Proponent</Label><Input id="assessment-author" value={form.author} onChange={(event) => updateForm('author', event.target.value)} className="mt-1" /></div>
              <div><Label htmlFor="assessment-school">School</Label><Input id="assessment-school" value={form.school} onChange={(event) => updateForm('school', event.target.value)} className="mt-1" /></div>
              <div><Label htmlFor="assessment-district">District</Label><Input id="assessment-district" value={form.district} onChange={(event) => updateForm('district', event.target.value)} className="mt-1" /></div>
              <div>
                <Label>Learning focus</Label>
                <Select value={form.learning_focus} onValueChange={(value) => updateForm('learning_focus', value)}>
              ...
              <div>
                ...
                <Select value={form.key_stage} onValueChange={updateKeyStage}>
              ...
              <div><Label htmlFor="assessment-grades">Grade levels</Label><Input id="assessment-grades" value={form.grade_levels} onChange={(event) => updateForm('grade_levels', event.target.value)} required className="mt-1" /></div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><Label htmlFor="framework-notes">Panel notes</Label><Textarea id="framework-notes" value={form.framework_notes} onChange={(event) => updateForm('framework_notes', event.target.value)} rows={4} className="mt-1" /></div>
